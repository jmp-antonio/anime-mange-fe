...
import { update } from '../../services/authorServices';
...
const EditAuthor = ({ author, openEditModal, setOpenEditModal, loadAuthorsTable }: Props) => {
    ...
    const onSubmit = async (values: Author) => {
        try {
            const { data: response } = await update(author.id, values);
